document.addEventListener('DOMContentLoaded', () => {
  ...
  const addHighlightTool = () => {
    ...
    const getActualHeight = item => {
      ...
      const fix = () => {
        ...
        if (hiddenElements && hiddenElements instanceof Map && hiddenElements.size > 0) {
    ...
    $figureHighlight && typeof $figureHighlight.forEach === 'function' && $figureHighlight.forEach(item => {
      let langName = ''
  ...
  const addPhotoFigcaption = () => {
    ...
    const articleImages = document.querySelectorAll('#article-container img')
    if (articleImages && typeof articleImages.forEach === 'function') {
  ...
  */
  const scrollFnToDo = () => {
    ...
    const findHeadPosition = top => {
      ...
      if (isToc) {
        $cardToc && $cardToc.querySelectorAll && $cardToc.querySelectorAll('.active').forEach(i => i.classList.remove('active'))
 ...
 */
  ...
  const relativeDate = selector => {
    if (!selector || typeof selector.forEach !== 'function') return
  ...
  window.addEventListener('hexo-blog-decrypt', e => {
    ...
    const encryptFns = window.globalFn && window.globalFn.encrypt ? Object.values(window.globalFn.encrypt) : []
    if (Array.isArray(encryptFns)) {
      encryptFns.forEach(fn => {
